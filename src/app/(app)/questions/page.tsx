...
import { QUESTION_STATUS_LABELS, QUESTION_STATUS_BADGE_CLASS } from "@/lib/constants";
      ...
      >
        {open.length === 0 ? (
          ...
          <ul className="divide-y">
            {open.map((q) => (
              <li key={q.id} className="px-5 py-4 transition-colors hover:bg-muted/40">
            ...
            {resolved.map((q) => (
              <li key={q.id} className="px-5 py-4 transition-colors hover:bg-muted/40">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span
                    className={`inline-flex shrink-0 items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${QUESTION_STATUS_BADGE_CLASS[q.status]}`}
                  >
